...
def run_workflow(
    ...
    *,
    ...
    # If the user wants to see every event, subscribe to all events
    if print_events:
        # Ensure the root logger is configured (in case this is called standalone)
        logging.basicConfig(
            level=getattr(logging, settings.LOG_LEVEL.upper(), logging.INFO),
            format=(
                "%(asctime)s | %(levelname)s | %(name)s | "
        ...
        # Note: Since this Workflow class does not support `subscribe`, this block
        # will have no effect unless subscribe is implemented. If `subscribe` is
        # missing, you can remove this block or leave it as a no-op.
        try:
